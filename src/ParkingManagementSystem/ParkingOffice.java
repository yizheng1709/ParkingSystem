...
public class ParkingOffice {
  ...
  public ParkingTransaction park(LocalDateTime in,LocalDateTime out, ParkingPermit p, ParkingLot l) {
    return transactionManager.park(in, out, p, l);
  }
  ...
  public final void setupParkingLots() {
    ...
    String[][] parkingLotData = { 
      { "W","Lot W","E Jewell Ave","","Denver","CO","80210","$10.00", "DC" },
      { "108","Lot 108","E Buchtel Ave","","Denver","CO","80210","$2.00", "HC" },
      { "321","Lot 321","S Gaylord St","", "Denver","CO","80210","$8.00", "DC" },
      { "301","Lot 301","E Evans Ave","","Denver","CO","80210","$8.00", "HC" },
    };
    ...
    for ( String[] row: parkingLotData ) {
      ...
      Address address = new Address.Builder()
          ...
          .withZip(row[6]).build();
      if(row[8] == "DC") {
    	  lot = new ParkingLot(row[0],row[1],address,Money.of(row[7]), chargeFactory.makeStrategy("dayNight"));
      }else {
    	  lot = new ParkingLot(row[0],row[1],address,Money.of(row[7]), chargeFactory.makeStrategy("longStay"));
      }
      addParkingLot(lot);
    }
  }
